/*
 * ============================================================================
 * TEST 14 — ABBA Protocol with Adaptive Cooldown
 ...
 * ============================================================================
 *
 *   The stack is symmetric about the micro-gap. Heat flows from the
 *   heater through the 3mm plate and glass slide to the gap interface.
 *   The TEGs sit on the OUTER side of each heater, measuring the
 *   temperature drop from the heater to the insulated 6mm base plate.
 *   TCs are mounted on the inner face of the 3mm plates, as close to
 *   the gap as possible, giving the best measure of interface temperature.
 *
 *       +---------------------------------------------+
 ...
 *     Top heater -> 3mm plate -> glass -> GAP -> glass -> 3mm plate
 ...
 *   4. Average TEG voltages during steady-state plateau.
 ...
 * GAP_DISTANCE:  Gap between glass slides ("contact", "3um", "6um", etc.)
 * MATERIAL_PAIR: Surfaces facing the gap ("SiO2-SiO2", "Au-SiO2", etc.)
 * SPACER_TYPE:   What defines the gap ("microsphere", "kapton_shim", "none")
...
//const char* RUN_ID          = "CAL_002";
//#const char* GAP_DISTANCE    = "contact";
//const char* MATERIAL_PAIR   = "SiO2-SiO2";
//const char* SPACER_TYPE     = "none";
//const int   RUN_NUMBER      = 1;
//const char* OPERATOR_NOTES  = "Adaptive cooldown, wait for ambient before next phase";

const char* RUN_ID          = "RECT_001";
const char* GAP_DISTANCE    = "508um";          // 20 mil = 2x10mil shims
const char* MATERIAL_PAIR   = "Steel-SiO2";
const char* SPACER_TYPE     = "kapton_shim_2x10mil";
const char* OPERATOR_NOTES  = "Steel on top, glass on bottom, 20mil air gap";
/* ============================================================================
 * TIMING & ADAPTIVE COOLDOWN THRESHOLDS
 * ============================================================================
 *
 * HEAT_DURATION_MS (20 min):
 *   Fixed heating time per phase. From TEST 13 data, steady state was
 *   reached at ~10 min (TEG dV/dt < 0.02 mV/s). 20 min provides ~10 min
 *   of plateau data for robust statistical averaging.
 *
 ...
 *   hanging forever. Normal cooldown from ~100 deg C takes 40-70 min.
 *
...
//const unsigned long HEAT_DURATION_MS   = 1200000UL;  // 20 minutes
 ...
 *   A1_FWD_HEAT   -> top heater ON for 20 min (forward direction)
 *   A1_COOLDOWN   -> heaters OFF, wait for plates to reach ambient
 *   B1_REV_HEAT   -> bottom heater ON for 20 min (reverse direction)
 *   B1_COOLDOWN   -> heaters OFF, wait for plates to reach ambient
 *   B2_REV_HEAT   -> bottom heater ON for 20 min (reverse repeat)
 *   B2_COOLDOWN   -> heaters OFF, wait for plates to reach ambient
 *   A2_FWD_HEAT   -> top heater ON for 20 min (forward repeat)
 *   A2_COOLDOWN   -> final cooldown
 *   COMPLETE       -> done, LED blinks
 *
 * Heating phases: fixed duration (HEAT_DURATION_MS)
 * Cooling phases: adaptive (wait for ambient temperature)
 */

enum ABBAPhase {
  PHASE_WAIT_AMBIENT,  // Pre-start: wait for room temperature
  PHASE_A1_HEAT,       // Forward heating #1 (top heater)
  PHASE_A1_COOL,       // Cooldown to ambient
  PHASE_B1_HEAT,       // Reverse heating #1 (bottom heater)
  PHASE_B1_COOL,       // Cooldown to ambient
  PHASE_B2_HEAT,       // Reverse heating #2 (bottom heater)
  PHASE_B2_COOL,       // Cooldown to ambient
  PHASE_A2_HEAT,       // Forward heating #2 (top heater)
...
void applyHeaterState() {
  switch (currentPhase) {
    case PHASE_A1_HEAT: case PHASE_A2_HEAT:
      setTop(true);  setBot(false);   // Forward: heat from top
      break;
    case PHASE_B1_HEAT: case PHASE_B2_HEAT:
      setTop(false); setBot(true);    // Reverse: heat from bottom
      break;
    default:
...
void setup() {
  ...
  Serial.println("# ADAPTIVE COOLDOWN VERSION (TEST 14)");
  Serial.println("# ============================================");
  ...
  Serial.println("# ABBA: WAIT -> A1 -> COOL -> B1 -> COOL -> B2 -> COOL -> A2 -> COOL");
  Serial.println("#");
 ...
 *      - Heating phases: advance after HEAT_DURATION_MS
